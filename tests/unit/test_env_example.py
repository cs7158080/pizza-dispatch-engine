...
The configuration surface is written in two places — the field lists in `pizza.config` and
the committed example — and two copies of one fact eventually disagree. This test is what
...
def test_env_example_matches_the_settings_classes() -> None:
    ...
    Compose assembles are covered without a settable line.
    """
